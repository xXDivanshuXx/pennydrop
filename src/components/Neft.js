...
const Neft = () => {
  ...
  const handleTxnExcel = () => {
    ...
    try{  
      ...
      const json = await ApiFile({"apiUrl":apiUrl, "method":method, "authAccess":authAccess, "bodyData":{ "user_id":userid, "mobile_no":phoneNumber, "senderNumber":senderNumber ,"name":name, "account_number":account, "account_type":accountType, "amount":selectedValue==="transfer" && checked || selectedValue==="transfer" && !checked ? amount : ammount , "mode":"NEFT", "remark":"testing", "is_verify":selectedValue=="verify" && checked ? 1 : checked && selectedValue=='transfer' ? 1 : 0, "fundTransfer":checked && selectedValue=='transfer' ? 1  : !checked && selectedValue=='transfer' ? 1 : 0, "bank_name":bankType?.bank_name, "ifsc_code":ifsc , "paymentType":"NEFT" } });
         ...
         sx={{ minHeight:"100vh" }} >
          ...
          <AppBar position='sticky' >
            ...
            <ArrowBackIosIcon sx={{ cursor:'pointer', color:'white', fontSize:"1.8rem" }} onClick={() => navigate(-1) } />
              ...
              <Typography sx={{ fontFamily:"montserrat", fontWeight:500, fontSize:'1.3rem' }} >N.E.F.T</Typography>
              </Box>
    ...
    />
      ...
      >
        <DialogTitle sx={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <CloseIcon onClick={() => window.location.reload() } sx={{ cursor:'pointer' , position:"absolute", right:10 }} />
        </DialogTitle>
      ...
      >
        <DialogTitle sx={{ display:'flex', alignItems:'center', justifyContent:'space-between' }} >
          ...
          <CloseIcon onClick={() => setImpsErrorDialogue(false)} sx={{ cursor:'pointer', position:"absolute", right:10 }} />
        </DialogTitle>
        <Divider variant='middle' />
        <DialogContent sx={{ display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:2 }} >
